...
const LocationAccordion = () => {
    ...
    return (
        <div className="accordion-preview-container">
        ...
        <p className="mb-5 meet-subheading">
          Come visit us for a coffee and a chat – we're always up for a good conversation!
                    ...
                    {/* Left Side - Accordion */}
                    <div className="col-12 col-lg-6 accordion-column">
                        
                        <div className="accordion-wrapper">
                            {accordionData.map((item) => (
                                <div key={item.id} className="accordion-item-wrapper">
                                    <button
                                        className={`accordion-header ${activeItem === item.id ? 'active' : ''}`}
                                        onClick={() => toggleAccordion(item.id)}
                                    >
                                        <span className="accordion-title"><IoLocationSharp />{item.locationName}</span>
                                        
                                        <span className={`accordion-icon ${expandedItem === item.id ? 'expanded' : ''}`}>
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                                                <path d="M5 7.5L10 12.5L15 7.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                            </svg>
                                        </span>
                                    </button>
                                    <div className={`accordion-content ${expandedItem === item.id ? 'expanded' : ''}`}>
